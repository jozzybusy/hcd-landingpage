...
const OptionWheel = ({
  ...
  const runFrame = useCallback(now => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // proximity: 1 at the horizontal center, 0 at/over one item away.
      ...
      // "Pop" scale: smoothly grow toward popScale near the center, with a
      // light overshoot for a bouncy feel. Disabled when popScale <= 1.
      let scale = 1;
      if (cfg.popScale > 1) {
        // easeOutBack-style curve so it slightly exceeds popScale before settling.
        const amp = cfg.popScale - 1;
        const overshoot = p * p * (2.4 * p - 1.4); // peaks ~1.06 at p≈0.78
        scale = 1 + amp * (p + 0.18 * overshoot);
      }
  ...
  }, [items, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, popScale, applyTarget]);
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          style={itemColors[index] ? { '--ow-active-color': itemColors[index] } : undefined}
        >
          {label}
        </div>
      ))}
